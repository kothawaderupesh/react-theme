...
import { AppMenuItem, MenuModel } from '@/types';

export const findTitleByPath = (menu: MenuModel[], path: string): string | null => {
    for (const item of menu) {
        if (item.to === path) {
            return item.label;
        }
        if (item.items) {
            const label = findTitleByPath(item.items, path);
...
export const AppMenuConfig: AppMenuItem[] = [
    {
        label: 'Home',
    ...
    },
    {
        ...
        items: [
            ...
            { label: 'Figma', url: 'https://www.dropbox.com/scl/fi/bhfwymnk8wu0g5530ceas/sakai-2023.fig?rlkey=u0c8n6xgn44db9t4zkd1brr3l&dl=0', icon: 'pi pi-fw pi-pencil', target: '_blank' },
            { label: 'View Source', icon: 'pi pi-fw pi-search', url: 'https://github.com/primefaces/sakai-react', target: '_blank' }
        ]
    }
];
